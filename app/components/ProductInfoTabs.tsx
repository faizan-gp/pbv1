...
export default function ProductInfoTabs({ description, features, careInstructions, sizeGuide }: ProductInfoTabsProps) {
    ...
    return (
        <div className="w-full">
            {/* FIX: Added 'overflow-x-auto', 'max-w-full', and 'flex-nowrap'
               This forces the tabs to scroll within their container on small screens
               instead of expanding the page.
            */}
            <div className="w-full overflow-x-auto scrollbar-hide mb-8 -mx-4 px-4 md:mx-0 md:px-0">
                <div className="inline-flex p-1.5 bg-slate-100 rounded-xl whitespace-nowrap">
                    {tabs.map((tab) => (
                        <button
                            key={tab.id}
